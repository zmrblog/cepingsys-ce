...
class AdminController
{
    ...
    public function destroy(Request $request, Response $response, array $args): Response
    {
        ...
        if ($id === $currentAdminId) {
            return error_response($response, 400, '不能删除当前登录的管理员');
        }

        if ($admin->role === 'super') {
            $superCount = DB::table('admins')
                ->where('role', 'super')
                ->where('status', 1)
                ->count();

            if ($superCount <= 1) {
                return error_response($response, 400, '至少保留一个超级管理员账户');
            }
        }
    ...
    public function logs(Request $request, Response $response): Response
    {
        $page = (int)($request->getQueryParams()['page'] ?? 1);
        $perPage = (int)($request->getQueryParams()['per_page'] ?? 20);
        $module = $request->getQueryParams()['module'] ?? null;
        $action = $request->getQueryParams()['action'] ?? null;

        $query = DB::table('operation_logs')
            ->leftJoin('admins', 'operation_logs.admin_id', '=', 'admins.id')
            ->select('operation_logs.*', 'admins.username', 'admins.real_name as admin_real_name');

        if ($module) {
            $query->where('operation_logs.module', $module);
        }

        if ($action) {
            $query->where('operation_logs.action', $action);
        }

        $total = (clone $query)->count();

        $logs = $query->orderBy('operation_logs.id', 'desc')
            ->offset(($page - 1) * $perPage)
            ->limit($perPage)
            ->get();
